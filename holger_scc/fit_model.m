...
for k = 1:numFolds
    ...
    %%%%%% changed by Holger to divide by length(n) instead of sum(n),
    %%%%%% because log-likelihoods are summed over time points
...
%     log_llh_test_mean = nansum(meanFR_test-n.*log(meanFR_test)+log(factorial(n)))/sum(n);
    log_llh_test_model = nansum(r-n.*log(r)+log(factorial(n)))/length(n); %note: log(gamma(n+1)) will be unstable if n is large (which it isn't here)
    log_llh_test_mean = nansum(meanFR_test-n.*log(meanFR_test)+log(factorial(n)))/length(n);
    ...
    %%%%%%% changed by Holger to divide by length(n_train) instead of
    %%%%%%% sum(n_train), because log-likelihoods are summed over time
    %%%%%%% points, not spike numbers
...
%     log_llh_train_mean = nansum(meanFR_train-n_train.*log(meanFR_train)+log(factorial(n_train)))/sum(n_train);
    log_llh_train_model = nansum(r_train-n_train.*log(r_train)+log(factorial(n_train)))/length(n_train);
    log_llh_train_mean = nansum(meanFR_train-n_train.*log(meanFR_train)+log(factorial(n_train)))/length(n_train);
