function [speed_grid,speedVec,speed] = speed_map(speed,nbins)
% original function: function [speed_grid,speedVec,speed] = speed_map(posx,posy,nbins)

%compute velocity
% sampleRate = 50;
% velx = diff([posx(1); posx]); vely = diff([posy(1); posy]); % add the extra just to make the vectors the same size

%%%%% replace the original code by the Kalman filter from the CMBHOME
%%%%% toolbox
% speed = sqrt(velx.^2+vely.^2).*sampleRate; 
% speed = CMBHOME.Utils.speed_kalman(velx, vely, sampleRate);
%%%%%%%%%%
